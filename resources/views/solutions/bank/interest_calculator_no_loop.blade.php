...
@section('title', 'Bank Interest Calculator (No Loop)')
    ...
    <div class="max-w-md w-full space-y-8 bg-white dark:bg-gray-800 p-10 rounded-xl shadow-2xl">
      <div>
        <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900 dark:text-gray-100">
          Bank Interest Calculator <span class="text-indigo-500">(No Loop/Formula)</span>
        </h2>
        <p class="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
          Calculates balance with 1% monthly interest using a direct formula. Initial balance: Rp 1,000,000.00
        </p>
      </div>
      ...
      <form class="mt-8 space-y-6" action="{{ route('bank.interest-no-loop.calculate') }}" method="POST">
      ...
      {{-- Display Result --}}
      @if (isset($balance))
        <div class="mt-8 p-6 bg-blue-50 dark:bg-blue-700 rounded-lg shadow">
          <h3 class="text-lg font-medium leading-6 text-blue-900 dark:text-blue-100">Calculation Result (No Loop)</h3>
          <div class="mt-2 text-lg text-blue-800 dark:text-blue-200">
      ...
      {{-- Test Cases (Same as problem 1.1) --}}
      <div class="mt-10">
        ...
        <p class="mt-4 text-xs text-gray-500 dark:text-gray-400">
          Note: The results should be identical to the loop version, demonstrating the formula's correctness.
        </p>
      </div>
